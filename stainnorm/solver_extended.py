...
from models import *
from layers import compare_ssim
from tools import get_updates, get_dataset
import tools as tools

from pca import PCAIterator
...
def compile(constructor, learning_rates, penalties, optimizer=nn.updates.nesterov_momentum):
  ...
  """
    ...
    structure_loss = (1 - compare_ssim(target_var, prediction).mean())*10
    # structure_loss = (1 - compare_ssim(target_var, prediction).mean()) * 500
    ...
    train_fn = theano.function([input_var, target_var], [error, loss, reg, structure_loss, prediction], updates=updates,
...
def run_solver(train_fn, network, layers, experiment_id, timestamp=None,
  ...
  """
    ...
    savedir = tools.require_dir("{}_{}_{}".format(str(dir_prefix), str(dataset), str(timestamp)))
    ...
    try:
        ...
        for epoch in range(100):
            ...
            for inputs, targets in datagen.iterate(X, batch_size=64, shuffle=True):
                ...
                ## TRAINING ##
                error, loss, reg, ssim, pred = train_fn(inputs, tools.crop(targets))
                ...
                log.info("#{}.{}: loss [{:.3f}, {:.3f}, {:.3f}, {:.3f}], outp range [{:.3f}, {:.3f}, {:.3f}]".format(
                    epoch, batch, float(error), float(loss), float(reg), float(ssim), pmin, pmean, pmax))
                losses.append(loss)
                regs.append(regs)
